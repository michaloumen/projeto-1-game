function init() {
  ...
  function checkProximity() {
    ...
    if (
      firstCell === secondCell + 1 ||
      firstCell === secondCell - 1 ||
      firstCell === secondCell - width ||
      firstCell === secondCell + width
    ) {
      proximity = true;
    } else {
  ...
  function checkColor() {
    ...
    while (i < arrayCheck.length && colorCheck === false) {
      if (
        arrayCheck[i][0] &&
        arrayCheck[i][1] &&
        arrayCheck[i][0].classList[1] === color &&
        arrayCheck[i][1].classList[1] === color
      ) {
        colorCheck = true;
      } else {
        colorCheck = false;
    ...
    for (let i = 0; i < cells.length; i++) {
      ...
      for (let i = 0; i < arrayCheck.length; i++) {
        if (
          ...
          arrayCheck[i][0] &&
          arrayCheck[i][1] &&
          cell.classList[1] === arrayCheck[i][0].classList[1] &&
          arrayCheck[i][0].classList[1] === arrayCheck[i][1].classList[1]
        ) {
  ...
  function createBoard() {
    ...
    scoreKeeper.innerHTML = `Score: ${score}`;
    movesKeeper.innerHTML =
      "You have 10 moves to gain 500 points, now crush that candy!";
    timeKeeper.innerHTML =
      "You have 1 minute to gain 1500 points, now crush that candy!";
